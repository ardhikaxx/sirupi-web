...
@section('title', 'Detail Verifikasi Paket')

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
    <li class="breadcrumb-item"><a href="{{ route('verifikator.paket.index') }}">Verifikasi Paket</a></li>
...
<div class="card border-0 shadow-sm rounded-4 mb-3">
    <div class="card-header bg-white border-0 pt-4 px-4 d-flex justify-content-between align-items-center">
        ...
        <a href="{{ route('verifikator.paket.index') }}" class="btn btn-outline-secondary btn-sm">
...
<div class="d-flex gap-2 justify-content-end">
    ...
    <button type="button" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#kembalikanModal">
...
<div class="modal fade" id="setujuiModal" tabindex="-1">
    <div class="modal-dialog">
        <form action="{{ route('verifikator.paket.setujui', $paket->id) }}" method="POST">
            @csrf
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Setujui & Teruskan</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <p>Anda akan menyetujui paket <strong>{{ $paket->nama_paket }}</strong> dan meneruskannya ke Pimpinan.</p>
                    <div class="mb-3">
...
<div class="modal fade" id="kembalikanModal" tabindex="-1">
    <div class="modal-dialog">
        <form action="{{ route('verifikator.paket.kembalikan', $paket->id) }}" method="POST" id="formKembalikan">
            @csrf
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Kembalikan Paket</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <p>Anda akan mengembalikan paket <strong>{{ $paket->nama_paket }}</strong> ke Operator untuk diperbaiki.</p>
                    <div class="mb-3">
                        <label class="form-label">Catatan <span class="text-danger">*</span></label>
                        <textarea name="catatan" id="catatanKembalikan" class="form-control" rows="3" placeholder="Minimal 10 karakter" required></textarea>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                    <button type="submit" class="btn btn-warning" id="btnKembalikan">Ya, Kembalikan</button>
                </div>
            </div>
        </form>
    </div>
...
<script>
    $('#formKembalikan').on('submit', function(e) {
        var catatan = $('#catatanKembalikan').val().trim();
        if (catatan.length < 10) {
            e.preventDefault();
            $('#catatanKembalikan').addClass('is-invalid');
            Swal.fire({
                icon: 'error',
                title: 'Validasi',
    ...
    $('#kembalikanModal').on('hidden.bs.modal', function() {
        $('#catatanKembalikan').removeClass('is-invalid');
    });
</script>
@endpush
