...
@section('content')
<form action="{{ route('operator.paket.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
        ...
        <div class="card-body px-4 pb-4">
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="tahun_anggaran_id" class="form-control select2 @error('tahun_anggaran_id') is-invalid @enderror" required>
                        ...
                        @foreach($tahunAnggarans as $ta)
                            <option value="{{ $ta->id }}" {{ old('tahun_anggaran_id') == $ta->id ? 'selected' : '' }}>{{ $ta->tahun }} - {{ $ta->nama }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-12">
                    ...
                    <textarea name="uraian_pekerjaan" rows="4" class="form-control @error('uraian_pekerjaan') is-invalid @enderror" required>{{ old('uraian_pekerjaan') }}</textarea>
                ...
                <div class="col-md-4">
                    ...
                    <select name="jenis_pengadaan_id" class="form-control select2 @error('jenis_pengadaan_id') is-invalid @enderror" required>
                        ...
                        @foreach($jenisPengadaans as $jp)
                            <option value="{{ $jp->id }}" {{ old('jenis_pengadaan_id') == $jp->id ? 'selected' : '' }}>{{ $jp->nama }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <select name="metode_pengadaan_id" class="form-control select2 @error('metode_pengadaan_id') is-invalid @enderror" required>
                        ...
                        @foreach($metodePengadaans as $mp)
                            <option value="{{ $mp->id }}" {{ old('metode_pengadaan_id') == $mp->id ? 'selected' : '' }}>{{ $mp->nama }}</option>
                        @endforeach
                    </select>
        ...
        <div class="card-body px-4 pb-4">
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="pagu_anggaran" id="pagu_anggaran" class="form-control @error('pagu_anggaran') is-invalid @enderror" value="{{ old('pagu_anggaran') }}" required>
                    @error('pagu_anggaran')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6">
                    <label class="form-label">HPS</label>
                    <input type="text" name="hps" id="hps" class="form-control @error('hps') is-invalid @enderror" value="{{ old('hps') }}">
                ...
                <div class="col-md-6">
                    ...
                    <select name="sumber_dana_id" class="form-control select2 @error('sumber_dana_id') is-invalid @enderror" required>
                        ...
                        @foreach($sumberDanas as $sd)
                            <option value="{{ $sd->id }}" {{ old('sumber_dana_id') == $sd->id ? 'selected' : '' }}>{{ $sd->nama }}</option>
                        @endforeach
                    </select>
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('operator.paket.index') }}" class="btn btn-outline-secondary">Batal</a>
    </div>
</form>
@endsection
...
<script>
    $(document).ready(function() {
        ...
        setTimeout(function() {
            $('#kegiatan_id').val('{{ old('kegiatan_id') }}').trigger('change');
            setTimeout(function() {
                $('#sub_kegiatan_id').val('{{ old('sub_kegiatan_id') }}');
            }, 500);
        }, 500);
        @endif
    });
